...
module.exports = function initialize (adapter, opts) {
  ...
  if (opts.network) {
    ...
    // network = ((opts.testnet) ? net.test : net.main).toBitcoinJS()
    network = (net.regtest).toBitcoinJS()  // TODO: change this statement
  } else {
    network = opts.testnet ? bitcoin.networks.testnet : bitcoin.networks.bitcoin
  }
